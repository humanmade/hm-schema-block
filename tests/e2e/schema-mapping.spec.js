...
test.describe( 'Schema.org Block Mapping', () => {
	...
	test( 'should allow adding attribute mappings', async ( {
		editor,
		page,
	} ) => {
		await editor.insertBlock( { name: 'core/group' } );
	...
	test( 'should allow mapping block content to schema property', async ( {
		editor,
		page,
	} ) => {
		...
		await expect( page.getByLabel( 'Source' ).first() ).toHaveValue(
			'content'
		);
	} );
} );
